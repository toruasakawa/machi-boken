...
function isSameCompletedSlopeSpot(a, b) {
  if (a.cellId && b.cellId && a.cellId === b.cellId) return true;
  ...
  return (
    ...
    COMPLETED_SLOPE_SPOT_DEDUPE_DISTANCE_M
  );
}

// 不正データ(壊れたJSON・型不一致)はアプリ全体を止めず、その地点だけ無視する。
function sanitizeCompletedSlopeSpots(raw) {
  if (!Array.isArray(raw)) return [];
  return raw.filter((spot) => {
    if (!spot || typeof spot !== "object") return false;
    if (!Number.isFinite(spot.lat) || !Number.isFinite(spot.lng)) return false;
    if (!spot.cellId && !spot.id) return false;
    if (spot.completedAt != null && !Number.isFinite(spot.completedAt)) return false;
    return true;
  });
}

// 手動編集・過去のバグ等で重複が紛れ込んでいた場合に備え、読み込み時にも1件へ整理する。
function dedupeCompletedSlopeSpots(spots) {
  const result = [];
  spots.forEach((spot) => {
    if (result.some((existing) => isSameCompletedSlopeSpot(existing, spot))) return;
    result.push(spot);
  });
  return result;
}

// 踏破済み地点は上限を設けず、明示的な削除操作以外では勝手に間引かない。
let completedSlopeSpots = dedupeCompletedSlopeSpots(
  sanitizeCompletedSlopeSpots(store.get(STORAGE_KEYS.completedSlopeSpots, [])),
);

// generateQuest()の候補選定(順位計算)には手を入れず、候補セルがこの条件に当てはまるかどうかだけを
// 判定する「後段フィルター」として使う（同じcellId、または保存済み地点から一定距離以内）。
function isNearCompletedSlopeSpot(lat, lng, cellId) {
  return completedSlopeSpots.some((spot) =>
    isSameCompletedSlopeSpot(spot, { lat, lng, cellId }),
  );
}
...
function saveCompletedSlopeSpot(quest) {
  const candidate = {
    ...
    score: quest.score != null ? quest.score : null, // 参考値。画面へ断定表示はしない
  };
  const alreadySaved = completedSlopeSpots.some((spot) =>
    isSameCompletedSlopeSpot(spot, candidate),
  );
  let persistedSuccessfully = true;
  if (!alreadySaved) {
    completedSlopeSpots = [...completedSlopeSpots, candidate];
    try {
      store.set(STORAGE_KEYS.completedSlopeSpots, completedSlopeSpots);
    } catch (e) {
      persistedSuccessfully = false; // 保存に失敗しても、今回セッションの達成表示自体は継続する
    }
  }
  return {
...
const completedSlopeSpotMarkersByCellId = new Map(); // spot.id -> Leafletマーカー（差分更新用）
let lastCompletedSlopeSpotsCenterCell = null; // 直近に描画した中心セル（無駄な再計算を防ぐ）
...
function initMap(lat, lon) {
  ...
  // 過去に踏破済みの旗。既定のmarkerPane(600、今回のクエスト旗が使う)よりわずかに下に置き、
  // 万一同じ場所で重なっても今回の候補旗が上に見えるようにする。タップは受け付ける(pointer-events既定のまま)。
  const completedSlopeSpotsPane = map.createPane("completedSlopeSpotsPane");
  completedSlopeSpotsPane.style.zIndex = 590;
  ...
  completedSlopeSpotsLayer = L.layerGroup().addTo(map);
...
// clothの中身は未踏破時は空、踏破済みは"✓"のみ（絵文字の見た目に依存せずCSSで形自体を作る）。
...
// 色だけに頼らず、状態ごとにaria-labelも変える（候補／接近中／踏破済み）。
function getSlopeQuestAriaLabel(status) {
  if (status === "completed") return `${SLOPE_QUEST_LABEL}踏破済み`;
  if (status === "nearby") return `${SLOPE_QUEST_LABEL}の近くです`;
  return `${SLOPE_QUEST_LABEL}候補`;
}
...
function drawQuestMarker() {
  ...
  if (q.status !== "ready" && q.status !== "nearby" && q.status !== "completed") return;
  if (slopeQuestCompletionInProgress) {
    // 踏破演出（チェック済み表示への切り替え）の途中で描き直さない。
...
// 冒険終了後も保存済み旗として残す）。踏破演出の途中(slopeQuestCompletionInProgress)であれば、
// 演出は最後まで自然に終わらせる。
...
function showSlopeQuestActionPanel() {
  const panel = el("slope-quest-action-panel");
  if (!panel) return;
  panel.hidden = false;
  const btn = el("complete-slope-quest-button");
  if (btn) btn.disabled = false;
  // フォーカスは勝手に移さない（地図を見ながら自分のペースで坂を進めるようにするため）。
}

function hideSlopeQuestActionPanel() {
  const panel = el("slope-quest-action-panel");
  if (panel) panel.hidden = true;
}

// GPS/セル接近判定は「踏破ボタンを表示してよいか」だけに使う。同一セル一致という判定方式自体は
// 変更しない（既存のbuildSlopeQuestArrivalDebugContextと同じ基準）。
function isNearSlopeQuest(currentCellId) {
  const q = adventureState.slopeQuest;
...
function markSlopeQuestArrivalEligible(curLat, curLon) {
  const q = adventureState.slopeQuest;
  if (q.arrivalEligible) return;
  q.arrivalEligible = true;
  q.status = "nearby";
...
// 踏破ボタン(complete-slope-quest-button)押下時の唯一の入口。ユーザーが実際にボタンを
// 押したときだけ達成を確定する（接近判定だけでは呼ばれない）。多重タップは押下直後の
...
function completeSlopeQuestManually() {
  ...
  if (quest.completedThisSession) return;

  quest.completedThisSession = true;
  quest.status = "completed";

  const completeBtn = el("complete-slope-quest-button");
  if (completeBtn) completeBtn.disabled = true; // 押下直後に無効化し、多重タップを防ぐ
...
/* ==========================================================
   ...
   ========================================================== */
function buildCompletedSlopeSpotIconHtml() {
  return `<span class="completed-slope-spot-marker__pole"></span><span class="completed-slope-spot-marker__cloth">✓</span>`;
}

function formatCompletedSlopeSpotDate(ts) {
  if (!Number.isFinite(ts)) return "";
  const d = new Date(ts);
  if (Number.isNaN(d.getTime())) return "";
  return `${d.getFullYear()}年${d.getMonth() + 1}月${d.getDate()}日に踏破`;
}
...
// 保存自体は全件残るが、常時全国分を描画しない（MVPでは現在地周辺描画で十分とする）。
function renderCompletedSlopeSpots(lat, lon) {
  if (!completedSlopeSpotsLayer) return;

  // 今回セッション自身の候補(ready/nearby/completedいずれか)は、questLayer側が表示を担当するため除外する。
  const q = adventureState.slopeQuest;
  const currentSessionCellId =
    q.status === "ready" || q.status === "nearby" || q.status === "completed"
      ? q.cellId
      : null;

  const nearby = completedSlopeSpots.filter((spot) => {
    if (currentSessionCellId && spot.cellId === currentSessionCellId) return false;
    return (
      haversineMeters(lat, lon, spot.lat, spot.lng) <=
      COMPLETED_SLOPE_SPOT_RENDER_RADIUS_M
    );
  });

  const nextIds = new Set(nearby.map((spot) => spot.id));
  // 範囲外になったものだけ削除する（重複防止・毎回全消去しないための差分更新）
  completedSlopeSpotMarkersByCellId.forEach((marker, id) => {
    if (nextIds.has(id)) return;
    completedSlopeSpotsLayer.removeLayer(marker);
    completedSlopeSpotMarkersByCellId.delete(id);
  });
  // 不足分だけ追加する
  nearby.forEach((spot) => {
    if (completedSlopeSpotMarkersByCellId.has(spot.id)) return;
    const icon = L.divIcon({
      className: "completed-slope-spot-marker",
      html: buildCompletedSlopeSpotIconHtml(),
      iconSize: [36, 36],
      iconAnchor: [18, 34],
    });
    const marker = L.marker([spot.lat, spot.lng], {
      pane: "completedSlopeSpotsPane",
      icon,
  ...
  logSlopeQuestPersistenceDebug({
    completedFlagsRendered: completedSlopeSpotMarkersByCellId.size,
  });
}

// 霧(updateFogCells)と同様、中心セルが変わった時だけ再計算する（GPS更新のたびに全消去・再構築しない）。
function updateCompletedSlopeSpotsIfNeeded(lat, lon) {
  if (completedSlopeSpots.length === 0 && completedSlopeSpotMarkersByCellId.size === 0) {
    return;
  }
  const { ix, iy } = cellIndex(lat, lon);
  if (
    lastCompletedSlopeSpotsCenterCell &&
    lastCompletedSlopeSpotsCenterCell.ix === ix &&
    lastCompletedSlopeSpotsCenterCell.iy === iy
  ) {
    return;
  }
  lastCompletedSlopeSpotsCenterCell = { ix, iy };
  renderCompletedSlopeSpots(lat, lon);
}
...
function updateHud(currentLat, currentLon) {
  ...
  const q = adventureState.slopeQuest;
  if (q.status === "ready" || q.status === "nearby") {
...
async function generateQuest(curLat, curLon) {
  ...
  for (let dx = -QUEST_RING_CELLS; dx <= QUEST_RING_CELLS; dx++) {
    for (let dy = -QUEST_RING_CELLS; dy <= QUEST_RING_CELLS; dy++) {
      ...
      const candidateCellId = cellKey(ix, iy);
      const c = cellCenterLatLon(ix, iy);
      // 踏破済み地点・近すぎる地点は、このあとの標高評価(順位計算)に入れる前に除外する後段フィルター。
      // 候補選定アルゴリズム自体(距離での絞り込み・勾配スコアの計算式)は変更しない。
      const exactMatch = completedSlopeSpots.some(
        (spot) => spot.cellId === candidateCellId,
      );
      if (exactMatch) {
        lastQuestGenerationExcludedAsCompleted++;
        continue;
      }
      const tooCloseToCompleted = completedSlopeSpots.some(
        (spot) =>
          haversineMeters(c.lat, c.lon, spot.lat, spot.lng) <=
          COMPLETED_SLOPE_SPOT_DEDUPE_DISTANCE_M,
      );
      if (tooCloseToCompleted) {
        lastQuestGenerationExcludedAsNearby++;
        continue;
      }
...
async function ensureQuest() {
  ...
  if (!picked) {
    ...
    // 除外後に候補が残らなかった場合。無理に平坦な場所や踏破済み地点を再利用しない。
    logSlopeQuestPersistenceDebug({ markerRedrawReason: "no-candidate-available" });
    return;
  }
  ...
  logSlopeQuestPersistenceDebug({ markerRedrawReason: "candidate-adopted" });
  drawQuestMarker();
...
const adventureState = {
  ...
  // 動かさない。GPS/セル接近判定は「踏破ボタンを表示してよいか」だけに使い、自動達成はしない。
  // status: idle(未選定) -> pending(検索中) -> ready(固定表示、未接近) -> nearby(接近判定成立、
  // 踏破ボタン表示中) -> completed(踏破ボタンを押した) または unavailable(有効な候補なし)。
  // ready/nearby/completed/unavailableでは同じ冒険中に再検索しない。
  slopeQuest: {
    status: "idle",
    lat: null,
    lng: null,
    ...
    score: null,
    requestId: null,
    arrivalEligible: false, // 一度trueになったら、ユーザーが踏破する/冒険を終える/新しい冒険を始めるまで維持する
    completedThisSession: false, // 終了画面バッジの表示条件。localStorageへは保存しない
  },
  completionData: null,
  currentCellId: null,
...
function resetAdventureStateKeepHistory() {
  ...
  // slopeQuest状態自体(completedThisSessionを含む)のリセットは新しい冒険開始時
  // (selectAdventurePreset)だけで行う。ここでは、冒険終了後すぐに演出タイマー(チェック済み化の
  // ポップ)を止め、次のセッションへ持ち越さないようにするだけ（実際のフラッグ削除はendAdventure側）。
  clearSlopeQuestCompletionTimers();
...
function endAdventure() {
  ...
  hideSlopeQuestActionPanel(); // 踏破案内パネルを閉じる
  if (!adventureState.slopeQuest.completedThisSession) {
    // 今回未踏破の候補旗は削除する。踏破済みなら消さず、保存済み旗としてそのまま残す
    // （次回以降はcompletedSlopeSpotsLayer側の通常表示に自然に切り替わる）。
...
// 接近判定→踏破ボタン→保存→再表示という、今回追加した一連の状態遷移専用のデバッグログ。
// arrivalThresholdMという名前は使わず、実際の判定方式(同一セル)をそのまま出す
// (buildSlopeQuestArrivalDebugContextのarrivalRule/approximateCellSizeMをspreadして使う)。
function logSlopeQuestPersistenceDebug(extra) {
  if (!DEBUG_SLOPE_QUEST) return;
  const q = adventureState.slopeQuest;
  console.log("[slope-quest-persistence]", {
    status: q.status,
    ...
    isSameCell: null,
    distanceToQuestM: null,
    completedSlopeSpotCount: completedSlopeSpots.length,
    candidateExcludedAsCompleted: lastQuestGenerationExcludedAsCompleted,
    candidateExcludedAsNearby: lastQuestGenerationExcludedAsNearby,
    savedSpotId: null,
    savedSpotCellId: null,
    persistedSuccessfully: null,
    currentQuestFlagVisible: !!questMarker,
    completedFlagsRendered: completedSlopeSpotMarkersByCellId.size,
    resultBadgeShown: null,
    ...extra,
  });
}
...
function showSlopeQuestNotification() {
  ...
  notification.textContent = SLOPE_QUEST_COMPLETION_MESSAGE;
  notification.classList.remove("hidden", "is-visible");
...
// 接近判定そのもの(同一セル一致)は既存仕様のまま。ログ用に、その判定根拠を明示的な形で残す。
// ※ここでの「到達」はあくまで踏破ボタンを表示してよいかの判定であり、自動達成ではない。
...
// 確定済み・保存も完了済み。ここでは演出(拡大→チェック済み化・紙吹雪・振動・通知)だけを行う。
function triggerSlopeQuestCompletion(saveResult) {
  ...
  pushLog("quest", `${SLOPE_QUEST_LABEL}を踏破 ${gradientLabel}`.trim());

  try {
    if ("vibrate" in navigator) navigator.vibrate(35);
  } catch (e) {
    // 振動非対応・失敗（iPhone Safari等）でも達成自体は伝わるようにする
  }
  ...
  const checkTimer = setTimeout(() => {
    ...
    // 達成後は旗を消さない。演出サイクルはここで終わり、演出中に保留されていた
    // 再描画要求があればfinishSlopeQuestCompletionCycle()内で反映する。
    finishSlopeQuestCompletionCycle();
  }, checkStateDelay);
  slopeQuestMarkerTimers.push(checkTimer);

  const debugContext = lastReliablePosition
    ? buildSlopeQuestArrivalDebugContext(lastReliablePosition.lat, lastReliablePosition.lon)
    : {};
  logSlopeQuestPersistenceDebug({
    ...debugContext,
    savedSpotId: saveResult ? saveResult.savedSpotId : null,
    savedSpotCellId: saveResult ? saveResult.savedSpotCellId : null,
    persistedSuccessfully: saveResult ? saveResult.persistedSuccessfully : null,
    markerRedrawReason: "completed",
  });
...
function getAdventureCompletionData() {
  ...
  return {
    ...
    selectedPresetMinutes: preset ? preset.minutes : null,
    goalReached: adventureState.goalReached,
    // 終了画面バッジは「今回踏破したか」だけを見る。過去の踏破済み旗が地図上にあるだけでは出さない。
    slopeQuestCompleted: adventureState.slopeQuest.completedThisSession,
